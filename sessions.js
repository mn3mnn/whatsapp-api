...
const { waitForNestedObject, checkIfEventisEnabled } = require('./utils')
...
const initializeEvents = (client, sessionId) => {
  ...
  checkIfEventisEnabled('auth_failure')
    .then(_ => {
      client.on('auth_failure', (msg) => {
        console.log(`${sessionId}, 'status', ${msg}`)
      })
    })
  ...
  checkIfEventisEnabled('call')
    .then(_ => {
      client.on('call', async (call) => {
        console.log(`${sessionId}, 'call', ${ call }`)
      })
    })
  ...
  checkIfEventisEnabled('change_state')
    .then(_ => {
      client.on('change_state', state => {
        console.log(`${sessionId}, 'change_state', ${ state }`)
      })
    })
  ...
  checkIfEventisEnabled('group_join')
    .then(_ => {
      client.on('group_join', (notification) => {
        console.log(`${sessionId}, 'group_join', ${ notification }`)
      })
    })
  ...
  checkIfEventisEnabled('group_leave')
    .then(_ => {
      client.on('group_leave', (notification) => {
        console.log(`${sessionId}, 'group_leave', ${ notification }`)
      })
    })
  ...
  checkIfEventisEnabled('group_update')
    .then(_ => {
      client.on('group_update', (notification) => {
        console.log(`${sessionId}, 'group_update', ${ notification }`)
      })
    })
  ...
  checkIfEventisEnabled('media_uploaded')
    .then(_ => {
      client.on('media_uploaded', (message) => {
        console.log(`${sessionId}, 'media_uploaded', ${ message }`)
      })
    })
        ...
        if (message.hasMedia && message._data?.size < maxAttachmentSize) {
          ...
          checkIfEventisEnabled('media').then(_ => {
            message.downloadMedia().then(messageMedia => {
              console.log(`${sessionId}, 'media', ${ messageMedia, message }`)
            }).catch(e => {
  ...
  checkIfEventisEnabled('message_ack')
    .then(_ => {
      client.on('message_ack', async (message, ack) => {
        console.log(`${sessionId}, 'message_ack', ${ message, ack }`)
        if (setMessagesAsSeen) {
          const chat = await message.getChat()
          chat.sendSeen()
        }
  ...
  checkIfEventisEnabled('message_create')
    .then(_ => {
      client.on('message_create', async (message) => {
        console.log(`${sessionId}, 'message_create', ${ message }`)
        if (setMessagesAsSeen) {
          const chat = await message.getChat()
          chat.sendSeen()
        }
  ...
  checkIfEventisEnabled('message_reaction')
    .then(_ => {
      client.on('message_reaction', (reaction) => {
        console.log(`${sessionId}, 'message_reaction', ${ reaction }`)
      })
    })
  ...
  checkIfEventisEnabled('message_revoke_everyone')
    .then(_ => {
      client.on('message_revoke_everyone', async (after, before) => {
        console.log(`${sessionId}, 'message_revoke_everyone', ${ after, before }`)
      })
    })
  ...
  client.on('qr', (qr) => {
    ...
    checkIfEventisEnabled('qr')
      .then(_ => {
        console.log(`SCAN QR FOR SESSION ${sessionId} `)
  ...
  checkIfEventisEnabled('ready')
    .then(_ => {
      client.on('ready', () => {
        console.log(`SESSION: ${sessionId} IS READY TO SEND MESSAGES`)
      })
    })
  ...
  checkIfEventisEnabled('contact_changed')
    .then(_ => {
      client.on('contact_changed', async (message, oldId, newId, isContact) => {
        console.log(`${sessionId}, 'contact_changed', ${ message, oldId, newId, isContact }`)
      })
    })
}
